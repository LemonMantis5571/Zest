...
//!
//! This is the small, local-first equivalent of TanStack AI's persistence
//! stores. Thread JSON remains the authoritative transcript; these records
//! answer the separate lifecycle questions: is a turn still running, did it
//! pause for approval, and how did it finish?
//!
//! The store contracts intentionally preserve the useful invariants from the
//! reference design:
//!
...
// callback can still update the same record concurrently. TanStack treats
// cross-instance coordination as a separate lock capability; this process-local
// mutex covers the equivalent race for the first local implementation. Atomic
// file replacement still protects the file if another process touches it.
