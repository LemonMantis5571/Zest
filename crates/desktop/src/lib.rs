...
///
/// Development launches from an external project (for example, a Vite app)
/// carry that project in the process working directory. Packaged launches are
/// commonly started in Zest's own install directory; that path is rejected by
/// `usable_workspace`, so normal last-workspace behavior remains intact there.
...
async fn send_message(
    ...
    // Keep the TanStack ordering invariant: the transcript is durable before a
